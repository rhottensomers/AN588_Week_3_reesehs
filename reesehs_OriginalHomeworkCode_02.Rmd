...
```{r, 8_calls}
ppois(7, lambda= 15, lower = FALSE) #ppois finds the cumulative probability so it is used in this context because more than one probability point is need 
#also instead of using 8 as the q value I used 7 because the question is asking for MORE THAN

1- ppois(q= 7, lambda= 15) #this also does the same thing as the above function 
```

# What is the probability that she will hear no calls in a session?
```{r, no_calls}
dpois(0, 15, log = FALSE) #dpois finds probability density at a certain point thus it was used to calculate the probability at 0 calls, this is also not in log for(hence log = FALSE), mean/lambda = 15
#I am not quite sure if this is how I am supposed to do this one or if I got the right answer?
```

# What is the probability that she will hear exactly 3 calls in a session?
```{r, 3_calls}
dpois(3, 15, log = FALSE)#finds probability for 3 using mean of 15 and not given as log
```
...
Sat_mon
#I initially was confusing myself by using rnorm function but they I was asking Jimmy about what the standard deviation would be and he let me know that I should really be using rpois instead since the data set is using poisson functions/data. So big thank yous to Jimmy they are the best!!
```
